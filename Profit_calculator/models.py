...
class InvestmentContext:
    ...
    def calculate_annual_interest_rate(self):
        ...
        if self.ter_fee > 0:
            annual_rate = (Decimal(1) + self.annual_interest_rate) * (Decimal(1) - self.ter_fee) - Decimal(1)
        # Apply annual tax rate of interests
        if self.tax_interest_rate > 0:
            annual_rate = (Decimal(1) + annual_rate) * (Decimal(1) - self.tax_interest_rate) - Decimal(1)
        return annual_rate
    ...
    def calculate_future_value(self, context: InvestmentContext):
        if self.deposit_type == "entry":
            ...
            if self.deposits_per_year <= 0:
                ...
                if context.interest_type == "nominal":
